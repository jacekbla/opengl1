...
private:
	...
	static std::vector<std::pair<glm::fvec2, glm::vec4>> createMeshData(int p_gridCount, int p_totalVertexCount);
	static void storeGridSquare(int p_col, int p_row, std::vector<std::pair<glm::fvec2, glm::vec4>>& p_data);
	static void storeTriangle(glm::fvec2* p_cornerPos, std::vector<std::pair<glm::fvec2, glm::vec4>>& p_data, bool p_left);
